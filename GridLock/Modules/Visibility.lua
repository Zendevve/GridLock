-- GridLock Visibility Module
-- Handles hiding and showing frames

local GridLock = select(2, ...)

local Visibility = {}
GridLock:RegisterModule("Visibility", Visibility)
...
function Visibility:OnInitialize()
    if GridLock.db then
        if not GridLock.db.hiddenFrames then
            GridLock.db.hiddenFrames = {}
        end
        self.hiddenFrames = GridLock.db.hiddenFrames
    end
...
function Visibility:ApplySavedHiddenStates()
    ...
    for frameName, data in pairs(self.hiddenFrames) do
        ...
        if frame and not GridLock.Utils.IsProtectedInCombat(frame) then
            frame:SetAlpha(0)
...
function Visibility:HideFrame(frameName)
    ...
    GridLock.Utils.Print(GRIDLOCK.FRAME_HIDDEN, frameName)
    return true
end
...
function Visibility:ShowFrame(frameName)
    ...
    GridLock.Utils.Print(GRIDLOCK.FRAME_SHOWN, frameName)
    return true
end
