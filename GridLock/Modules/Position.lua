...
function Position:SavePosition(frameName, frame)
    ...
    local posData = GridLock.Utils.SerializePoint(frame, 1)
    if not posData then return false end

    -- Retain existing scale and alpha if present in DB
    local existing = GridLock:GetFramePosition(frameName)
    if existing then
    ...
    -- Save to database
    GridLock:SaveFramePosition(frameName, posData)
    ...
    GridLock.Utils.Debug("Saved position for %s", frameName)
    return true
end
...
function Position:SerializeAllPoints(frame)
    ...
    for i = 1, frame:GetNumPoints() do
        local pointData = GridLock.Utils.SerializePoint(frame, i)
        if pointData then
...
function Position:DisableManagedPosition(frameName)
    ...
    if UIPARENT_MANAGED_FRAME_POSITIONS and UIPARENT_MANAGED_FRAME_POSITIONS[frameName] then
        ...
        GridLock.Utils.Debug("Disabled managed position for %s", frameName)
    end
...
function Position:ApplyPosition(frameName, posData)
    ...
    if not posData then
        posData = GridLock:GetFramePosition(frameName)
    end
    if not posData then return false end

    -- Check combat lockdown
    if GridLock.Utils.IsProtectedInCombat(frame) then
        GridLock.Utils.Debug("Cannot apply position to %s during combat", frameName)
        return false
    end
    ...
    local applyPoint = frame.GridLockOriginalSetPoint or frame.SetPoint
    local clearPoints = frame.GridLockOriginalClearAllPoints or frame.ClearAllPoints
    ...
    GridLock.Utils.Debug("Applied position to %s", frameName)
    return true
end

-- Apply all saved positions
function Position:ApplyAllSavedPositions()
    for frameName, posData in pairs(GridLock.db.frames) do
...
function Position:ResetPosition(frameName)
    ...
    local defaultPos = GridLock.PositionData and GridLock.PositionData:GetDefaultPosition(frameName)

    if origData and origData.points and #origData.points > 0 then
        for _, pointData in ipairs(origData.points) do
            GridLock.Utils.ApplyPoint(frame, pointData)
        end
    ...
    GridLock:ClearFramePosition(frameName)
...
function Position:ResetAll()
    ...
    local framesToReset = {}
    if GridLock.db and GridLock.db.frames then
        for frameName in pairs(GridLock.db.frames) do
    ...
    if GridLock.db then
        if GridLock.db.frames then wipe(GridLock.db.frames) end
        if GridLock.db.hiddenFrames then wipe(GridLock.db.hiddenFrames) end
    end
    ...
    GridLock.Utils.Print(GRIDLOCK.POSITION_RESET_ALL or "All frame positions, scales, and visibilities have been reset.")
end
...
function Position:HookFrame(frameName, frame)
    ...
    if frame.GridLockHooked then return end

    -- Store originals
    local originalSetPoint = frame.SetPoint
    local originalClearAllPoints = frame.ClearAllPoints

    frame.GridLockOriginalSetPoint = originalSetPoint
    frame.GridLockOriginalClearAllPoints = originalClearAllPoints
    ...
    frame.SetPoint = function(self, ...)
        ...
        if name and Position.hookedFrames[name] then
            local posData = GridLock:GetFramePosition(name)
            if posData then
    ...
    frame.GridLockHooked = true
    GridLock.Utils.Debug("Hooked frame: %s", frameName)
end
...
function Position:UnhookFrame(frameName)
    ...
    if frame.GridLockHooked then
        if frame.GridLockOriginalSetPoint then
            frame.SetPoint = frame.GridLockOriginalSetPoint
            frame.GridLockOriginalSetPoint = nil
        end
        if frame.GridLockOriginalClearAllPoints then
            frame.ClearAllPoints = frame.GridLockOriginalClearAllPoints
            frame.GridLockOriginalClearAllPoints = nil
        end
        frame.GridLockHooked = nil
    end

    GridLock.Utils.Debug("Unhooked frame: %s", frameName)
end
...
function Position:IsModified(frameName)
    return GridLock:HasSavedPosition(frameName)
end
