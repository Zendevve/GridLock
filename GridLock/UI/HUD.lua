...
function HUD:CreateFrame()
    ...
    gridBtn:SetText("Grid: " .. (GridLock.db and GridLock.db.gridSize or 32) .. "px")
    ...
    snapBtn:SetText((GridLock.db and GridLock.db.snapEnabled) and "Snap: ON" or "Snap: OFF")
    snapBtn:SetScript("OnClick", function(self)
        GridLock.db.snapEnabled = not GridLock.db.snapEnabled
        self:SetText(GridLock.db.snapEnabled and "Snap: ON" or "Snap: OFF")
        GridLock.Utils.Print(GridLock.db.snapEnabled and GRIDLOCK.SNAP_ENABLED or GRIDLOCK.SNAP_DISABLED)
    end)
    f.snapBtn = snapBtn
...
function HUD:Show()
    ...
    if self.frame.snapBtn then
        self.frame.snapBtn:SetText((GridLock.db and GridLock.db.snapEnabled) and "Snap: ON" or "Snap: OFF")
    end
