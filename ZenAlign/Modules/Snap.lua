...
function Snap:CreateGuideOverlayFrame()
    if not self.guideFrame then
        local f = CreateFrame("Frame", "ZenAlignSnapGuideOverlay", UIParent)
...
function Snap:CalculateSnappedPosition(x, y, gridSize)
    gridSize = gridSize or (ZenAlign.db and ZenAlign.db.gridSize) or 32
...
function Snap:CheckCenterSnap(x, y, threshold)
    threshold = threshold or (ZenAlign.db and ZenAlign.db.snapThreshold) or 10
...
function Snap:GetCandidateTargetFrames(draggedFrame)
    ...
    local function addCandidate(frame)
        ...
        if left and bottom then
            ...
            local cx, cy = ZenAlign.Utils.GetFrameCenter(frame)
    ...
    if ZenAlign.FrameData and ZenAlign.FrameData.frames then
        for cat, frameList in pairs(ZenAlign.FrameData.frames) do
...
function Snap:GetFrameToFrameSnap(draggedFrame, rawX, rawY)
    ...
    local snapThreshold = (ZenAlign.db and ZenAlign.db.snapThreshold) or 10
    local releaseThreshold = (ZenAlign.db and ZenAlign.db.snapReleaseThreshold) or 16
...
function Snap:GetSnappedPosition(x, y, frameWidth, frameHeight, frame)
    if not ZenAlign.db or not ZenAlign.db.snapEnabled then
        return x, y, false
    end
    ...
    if frame and ZenAlign.db.snapToFrames ~= false then
    ...
    local threshold = (ZenAlign.db and ZenAlign.db.snapThreshold) or 10
    ...
    if not snappedX or not snappedY then
        local gridSize = ZenAlign.db.gridSize or 32
...
function Snap:ShowGuides(x, y, showVertical, showHorizontal)
    ...
    local threshold = (ZenAlign.db and ZenAlign.db.snapThreshold or 10) * 2
    if math.abs(x - centerX) < threshold then
