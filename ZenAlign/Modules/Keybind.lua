-- ZenAlign Keybind Module
-- Interactive Hover Keybinding Engine for WoW 3.3.5a

local addonName, ZenAlign = ...
ZenAlign = ZenAlign or _G.ZenAlign

local Keybind = {}
ZenAlign:RegisterModule("Keybind", Keybind)
...
function Keybind:CreateHighlightFrame()
    ...
    local f = CreateFrame("Frame", "ZenAlignKeybindHighlight", UIParent)
...
function Keybind:CreateCatcher()
    ...
    local catcher = CreateFrame("Button", "ZenAlignKeybindCatcher", UIParent)
...
function Keybind:Activate()
    ...
    if ZenAlign.Utils and ZenAlign.Utils.Print then
        ZenAlign.Utils.Print("Keybind mode activated. Hover over action buttons and press keys to bind. Press ESC to exit.")
    end
end
...
function Keybind:Deactivate()
    ...
    if ZenAlign.Utils and ZenAlign.Utils.Print then
        ZenAlign.Utils.Print("Keybind mode deactivated.")
    end
end
...
function Keybind:BindKeyToButton(button, keyString)
    ...
    if ZenAlign.Utils and ZenAlign.Utils.Print then
        ZenAlign.Utils.Print("Bound %s to %s (%s)", keyString, self:GetButtonActionName(button), command)
    end
end
...
function Keybind:ClearButtonBindings(button)
    ...
    if ZenAlign.Utils and ZenAlign.Utils.Print then
        ZenAlign.Utils.Print("Cleared bindings for %s (%s)", self:GetButtonActionName(button), command)
    end
end
