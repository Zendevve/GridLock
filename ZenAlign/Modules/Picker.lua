...
function Picker:CreateHighlightFrame()
    ...
    local f = CreateFrame("Frame", "ZenAlignPickerHighlight", UIParent)
...
-- Check if frame is a ZenAlign internal frame that should be ignored
function Picker:IsZenAlignFrame(frame)
    if not frame then return true end
    local name = frame:GetName() or ""
    if name:find("^ZenAlign") then
        return true
    end
    return false
end
...
function Picker:Start()
    ...
    if not self.clickCatcher then
        local catcher = CreateFrame("Button", "ZenAlignPickerCatcher", UIParent)
    ...
    ZenAlign.Utils.Print(ZENALIGN.PICKER_STARTED or "Frame picker active. Click any UI element to edit (Right-click or ESC to cancel).")
end
...
function Picker:Stop()
    ...
    ZenAlign.Utils.Print(ZENALIGN.PICKER_STOPPED or "Frame picker stopped.")
end
...
-- Mouse trace update loop (bypasses ZenAlignPickerCatcher to find target frame)
function Picker:OnUpdate()
    ...
    if focus and focus ~= UIParent and focus ~= WorldFrame then
        ...
        while curr and curr ~= UIParent and curr ~= WorldFrame do
            ...
            if currName and ZenAlign.FrameData and ZenAlign.FrameData:IsKnownFrame(currName) then
                focus = curr
                name = currName
                break
            end
        ...
        if name and not self:IsZenAlignFrame(focus) then
            ...
            if x and y then
                ...
                local frameInfo = ZenAlign.FrameData and ZenAlign.FrameData:GetFrameInfo(name)
                if frameInfo then
